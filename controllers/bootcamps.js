...
exports.createBootcamp = async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `The user ID ${req.user.id} is already published a bootcamp`,
        400
      )
    );
  }
...
exports.getBootcampsInRadius = async (req, res, next) => {
  console.log('enter here'.bgRed.inverse);
  ...
  //calc raius using radians
  // divide dist by radius Of earth
  //earth radius = 3,963 mi

  const radius = distance / 3964;
...
exports.bootcampPhotoUpload = async (req, res, next) => {
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (err) => {
    if (err) {
      console.error(err);
      return new ErrorResponse(`problem with upload `, 500);
    }
